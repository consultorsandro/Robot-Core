...
Version 1.2 preserves the decisions established during Phase 4.1 —
C++ Types & Contracts and incorporates the spatial-model refinement
discovered during Phase 4.2 — C++ Implementation & Testing.
...
where each Coordinate represents a finite signed location along one axis
of the fixed World Frame, expressed in meters. Negative coordinates
represent locations on the negative side of the corresponding axis
relative to its origin; they are not negative physical lengths.
...
unstructured collection of unrelated numeric values. Each axis component
is a Coordinate.

Coordinates are signed because a point may lie on either side of the
World Frame origin along each axis. A negative coordinate identifies the
negative side of the corresponding axis and is not inherently dangerous.

For example, a negative Z coordinate may identify a point below the
World Frame origin. Position and Coordinate do not determine whether
that point represents a safe lower floor, stairs, a ramp, a hole, a drop,
a precipice, an obstacle, or traversable or non-traversable terrain.
Those interpretations belong to future perception, environment modeling,
planning, or safety responsibilities and remain outside the current
...
-   perform environmental or safety interpretation;
...
Coordinate represents a signed position along one axis of the fixed
World Frame, expressed in meters.

### Semantic model

``` text
Coordinate < 0
    -> negative side of the axis origin

Coordinate = 0
    -> at the axis origin

Coordinate > 0
    -> positive side of the axis origin
```
...
-   Zero is valid.
-   Positive finite values are valid.
-   NaN is invalid.
...
Coordinate must be a strong domain type, not a simple alias such as:

``` cpp
using Coordinate = double;
```

The initial implementation may use double internally, but the underlying
numeric representation remains an implementation detail.

The semantic distinction is:

``` text
Length
    -> non-negative physical magnitude

Coordinate
    -> signed spatial location relative to an origin
```

Coordinate contains no environmental or safety interpretation. It does
not perform localization, perception, terrain classification, planning,
or robot behavior.
...
Length represents a non-negative physical magnitude in meters.

A valid Length is finite and greater than or equal to zero:
...
Length represents magnitude, not signed location. It must not be used
directly as a signed Cartesian coordinate. This refinement preserves the
implemented Length contract and does not weaken its invariant.
...
Core/domain logic may depend on stable domain concepts and abstractions.
Concrete infrastructure implementations may depend on and implement those
abstractions. The dependency relationships are:

``` text
Core / Domain Logic
    |
    +---- depends on ----> Stable Domain Concepts / Abstractions

Infrastructure Implementations
    |
    +---- implement / depend on ----> Stable Abstractions
```

Application is the composition root. It may know both core components
and concrete infrastructure in order to construct and connect the system:
...
For example, a concrete actuator implementation can implement the
core-facing Actuator abstraction. RobotCore depends on that abstraction,
not on the concrete implementation. Application constructs the selected
implementation and injects it into RobotCore. Application retains
ownership and lifetime responsibility for these components; RobotCore
uses its injected dependencies without owning or constructing them.

Dependency direction is distinct from the runtime data flow in Section 5.
Commands may reach a concrete backend through the Actuator boundary at
runtime without core/domain logic depending on that concrete backend.
...
-   represent Position axes using the signed Coordinate domain type;
-   distinguish signed spatial coordinates from non-negative physical
    lengths;
...
-   Length represents a valid non-negative physical magnitude in meters.
-   Coordinate represents a valid finite signed World Frame axis value in
    meters.
-   Position represents a 3D World Frame position using signed
    coordinates.
...
This document represents Architecture Baseline v1.2.
...
8.  be published with Git Push after review as a stable completed
    development increment.

The architecture is therefore a living baseline, not a permanent
constraint.

## 14.1 v1.2 spatial-model refinement — 2026-09-02

### Problem

During Phase 4.2 — C++ Implementation & Testing, implementation of
Length exposed a mismatch in v1.1: Position used Length for its
components, even though Length is non-negative and Cartesian
coordinates require signed values.

### Alternatives considered

1.  Allow Length to contain negative values. Rejected because this would
    weaken the semantic invariant of physical magnitude.
2.  Use raw signed numeric values directly inside Position. Rejected
    because this would lose strong domain typing and semantic protection.
3.  Introduce a dedicated signed Coordinate strong domain type. Chosen
    because it preserves the distinction between magnitude and location.

### Chosen solution

Introduce Coordinate as a dedicated signed spatial domain type and use
three Coordinates in Position: x, y, and z.

### Consequences

-   Length remains unchanged.
-   Coordinate may represent negative, zero, or positive finite positions.
-   Position supports the full signed Cartesian coordinate space required
    by the initial World Frame model, within the finite numeric
    representation.
-   Environmental and safety interpretation remains outside these value
    types.
-   Implementation order becomes Length -> Coordinate -> Position,
    followed by the remaining approved domain and component increments.
...
Phase 4.2 — C++ Implementation & Testing is in progress.

The Length increment is complete:

-   Length strong domain type implemented;
-   Length invariant validation implemented;
-   Length unit tests implemented;
-   GoogleTest and CTest integrated;
-   `robot_core` static library structure established;
-   Length increment successfully built and tested;
-   Length increment committed and published.

The next implementation increment is Coordinate, followed by Position.
Coordinate must go through the normal engineering workflow below before
Position implementation begins.
...
Push represents publication of a reviewed, stable completed development
increment. GitHub activity is not a goal by itself.
...
Version 1.2 preserves the Phase 4.1 architecture and records the signed
coordinate refinement discovered during Phase 4.2: Position uses three
Coordinates, while Length remains a non-negative physical magnitude.
Future robotics capabilities remain outside the current implementation
requirements.
